...
func TestValidateBFullManifestRejectsStructuredEvidenceClaimsWithoutExecution(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "alpha chain omitted", mutate: func(evidence *BFullBatchEvidence) {
			evidence.FirstPaint[0].Raw.Text[0].BackgroundChain = nil
		}, wantErr: "actual composited background chain"},
		{name: "static frame motion", mutate: func(evidence *BFullBatchEvidence) {
			evidence.FirstPaint[0].Raw.Motion.ObservedDeltaMS = 0
		}, wantErr: "motion action outcome is static"},
