...
func cdnURL(d dep) string {
	return strings.ReplaceAll(d.URL, "{v}", d.Version)
}
...
func populateIntegrities(deps map[string]dep) error {
	for module, dependency := range deps {
		contents, err := os.ReadFile(diskPath(module, dependency))
		if err != nil {
			return fmt.Errorf("read vendored dependency for integrity: %w", err)
		}
...
func render(deps map[string]dep) string {
	...
	for module, d := range deps {
		...
		rows = append(rows, kv{strings.TrimSuffix(name, "URL") + "CDNURL", cdnURL(d)})
		rows = append(rows, kv{strings.TrimSuffix(name, "URL") + "Integrity", d.Integrity})
	}
	sort.Slice(rows, func(i, j int) bool { return rows[i].name < rows[j].name })

	var buf bytes.Buffer
	buf.WriteString("// Code generated by vendorgen. DO NOT EDIT.\n")
	buf.WriteString("// Source of truth: assets/js/runtime/versions.json and vendored bytes\n\n")
...
func Run(args []string, stdout, stderr io.Writer) error {
	...
	if err := populateIntegrities(deps); err != nil {
		return err
	}
