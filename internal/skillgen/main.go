...
func render(pkgs []pkgAPI) string {
	...
	b.WriteString("## Component model\n\n")
	b.WriteString("A **Theme** selects the token set for a page or application and is independent of\n")
	b.WriteString("component identity. A **Primitive** is one meaningful renderable concept. Its\n")
	b.WriteString("concrete `components.Component` returns its stable **Kind** from `Kind()`. A\n")
	b.WriteString("**configuration dimension** is one independent choice such as `Tone`,\n")
	b.WriteString("`Appearance`, `Size`, `Orientation`, `Position`, or `Mode`; it is documentation\n")
	b.WriteString("vocabulary, not a public `Dimension` type. See\n")
