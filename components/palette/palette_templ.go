...
func paletteTemplate(cfg Config) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if cfg.ShowHex {
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 13, "<div class=\"flex items-center gap-2 pt-1\"><label data-selected-preview class=\"relative size-7 overflow-hidden rounded border border-control-outline shadow-sm dark:border-control-outline-dark cursor-pointer\" title=\"Custom color\" :style=\"{ backgroundColor: selectedHex }\"><input type=\"color\" :value=\"selectedHex\" @input=\"commitHex($event.target.value)\" @change=\"commitHex($event.target.value)\" class=\"absolute inset-0 size-full cursor-pointer opacity-0\" aria-label=\"Custom color\"></label> <input type=\"text\" x-model=\"hexInput\" pattern=\"^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$\" placeholder=\"#000000\" @input=\"previewHex(hexInput)\" @change=\"commitHex(hexInput)\" @keydown.enter.prevent=\"commitHex(hexInput)\" @blur=\"commitHex(hexInput)\" :aria-invalid=\"hexInvalid ? 'true' : 'false'\" class=\"w-24 rounded-radius border border-control-outline bg-surface px-2 py-1 text-xs font-mono text-on-surface focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary aria-[invalid=true]:border-danger aria-[invalid=true]:text-danger dark:border-control-outline-dark dark:bg-surface-dark dark:text-on-surface-dark dark:focus-visible:outline-primary-dark\"></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
...
func swatchGrid(cfg Config) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if !cfg.HideNeutral {
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 16, "<button type=\"button\" data-cls=\"white\" @click=\"pick($el.dataset.cls, $el)\" @mouseenter=\"hovered = $el.dataset.cls\" @mouseleave=\"hovered = ''\" @focus=\"hovered = $el.dataset.cls\" @blur=\"hovered = ''\" class=\"h-5 w-full rounded-sm border border-control-outline bg-white transition-transform hover:scale-125 hover:ring-2 hover:ring-primary focus:scale-125 dark:border-control-outline-dark dark:hover:ring-primary-dark\" title=\"white\"></button> <button type=\"button\" data-cls=\"black\" @click=\"pick($el.dataset.cls, $el)\" @mouseenter=\"hovered = $el.dataset.cls\" @mouseleave=\"hovered = ''\" @focus=\"hovered = $el.dataset.cls\" @blur=\"hovered = ''\" class=\"h-5 w-full rounded-sm border border-control-outline bg-black transition-transform hover:scale-125 hover:ring-2 hover:ring-primary focus:scale-125 dark:border-control-outline-dark dark:hover:ring-primary-dark\" title=\"black\"></button> ")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
		for _, hue := range cfg.hues() {
			for _, shade := range cfg.shades() {
				...
				templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 18, "\" @click=\"pick($el.dataset.cls, $el)\" @mouseenter=\"hovered = $el.dataset.cls\" @mouseleave=\"hovered = ''\" @focus=\"hovered = $el.dataset.cls\" @blur=\"hovered = ''\" class=\"h-5 w-full rounded-sm border border-control-outline dark:border-control-outline-dark transition-transform hover:scale-125 hover:ring-2 hover:ring-primary focus:scale-125 dark:hover:ring-primary-dark\"")
				if templ_7745c5c3_Err != nil {
					return templ_7745c5c3_Err
				}
