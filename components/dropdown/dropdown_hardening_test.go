...
func TestDropdownItemRendersCaptionAndConsumerLinkMetadata(t *testing.T) {
	rendered := renderDropdown(t, Config{Sections: []Section{{Items: []Item{{
		Label:        "Open docs",
		Caption:      "View this page in a new tab",
		Href:         "https://example.com/docs",
		Target:       "_blank",
		Rel:          "noopener noreferrer",
		TrailingIcon: testDropdownIcon("external"),
		Icon:         testDropdownIcon("docs"),
	}}}}})

	for _, want := range []string{
		`Open docs`,
		`View this page in a new tab`,
		`target="_blank"`,
		`rel="noopener noreferrer"`,
		`data-icon="docs"`,
		`data-icon="external"`,
...
func TestDropdownItemIgnoresLinkMetadataWhenRenderedAsButton(t *testing.T) {
	rendered := renderDropdown(t, Config{Sections: []Section{{Items: []Item{{
		Label:   "Run",
		Href:    "/run",
		Target:  "_blank",
	...
	assert.Contains(t, rendered, `x-on:click="run()"`)
	assert.NotContains(t, rendered, `href="/run"`)
	assert.NotContains(t, rendered, `target="_blank"`)
	assert.NotContains(t, rendered, `rel="noopener noreferrer"`)
}

func TestDropdownButtonItemsAlignContentWithLinks(t *testing.T) {
	rendered := renderDropdown(t, Config{Sections: []Section{{Items: []Item{{
		Label:   "Schedule",
		Caption: "Choose a release time",
		OnClick: "schedule()",
	}}}}})

	assert.Contains(t, rendered, `text-left`)
}

func TestDropdownLabeledTriggerRendersLeadingIcon(t *testing.T) {
	rendered := renderDropdown(t, Config{
		Label:       "Actions",
	...
	assert.Contains(t, rendered, `Actions<svg`)
}
...
func TestDropdownKeyboardOpenFocusesFirstEnabledItemAndEscapeReturnsToTrigger(t *testing.T) {
	...
	assert.Contains(t, rendered, `openFromKeyboard()`)
	...
	assert.Contains(t, rendered, `x-data="goshtosoPopover($el)"`)
