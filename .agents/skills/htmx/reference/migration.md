...
Run relevant unit tests, full E2E tests and regeneration gates from AGENTS.md for the actual migration. Verify both `just site-current-source-integration` and `just site-pinned-dependency-deployability`; follow `docs/SITE_MODULE_CONTRACTS.md` when the site adopts an unreleased root API. Keep runtime, event/markup changes and public compatibility documentation coordinated in release planning.
