...
**This repo (Goshtoso) is a templ component library** (Go + templ + Tailwind + HTMX + Alpine.js). Generated `*_templ.go` files are **never hand-edited** — regenerate. See `reference/gotchas.md` and the project `CLAUDE.md` for the repo's hard-won escaping rules before generating any Alpine/htmx markup.
...
- **`reference/gotchas.md`** — **read before generating Alpine.js/htmx markup.** templ escaping silently breaks `x-data`/`hx-*` (no console error); `json.Marshal` in attributes; null-array crashes; regeneration quirks. Cross-references repo `CLAUDE.md`.
