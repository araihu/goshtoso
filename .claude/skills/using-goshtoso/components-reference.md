...
40 component packages. Each is imported by its directory path; note the
...
**Entry points:** `Combobox(cfg Config)`

- **SelectionMode** — ModeSingle = "single", ModeMultiple = "multiple"
- **Size** — SizeSM = "sm", SizeMD = "md", SizeLG = "lg"
...
| Field | Type | Description |
|-------|------|-------------|
| `ID` | `string` | ID is a unique identifier for the combobox |
| `Name` | `string` | Name is the form field name |
| `Label` | `string` | Label is the label text shown above the combobox |
| `Placeholder` | `string` | Placeholder text when no selection |
| `State` | `State` | State is the validation state (error, success, or default) |
| `Options` | `[]Option` | Options is the list of available options (for static data) |
| `Selected` | `[]string` | Selected contains the currently selected values |
| `Mode` | `SelectionMode` | Mode determines single or multi-select |
| `EnableSearch` | `bool` | EnableSearch adds a search field to filter options |
| `SearchPlaceholder` | `string` | SearchPlaceholder text for the search field |
| `EnableClearAll` | `bool` | EnableClearAll shows a "Clear all" button when items are selected |
| `ClearAllText` | `string` | ClearAllText is the text for the clear all button (default: "Clear all") |
| `Size` | `Size` | Size of the combobox |
| `Disabled` | `bool` | Disabled disables the combobox |
| `HXMTEndpoint` | `string` | HTMX endpoint for lazy loading options |
| `HTMXTrigger` | `string` | HTMXTrigger for loading options (default: "click") |
| `HTMXSearchParam` | `string` | HTMXSearchParam is the query param name for search (default: "search") |
| `Class` | `string` | Class allows additional CSS classes |
| `NoResultsText` | `string` | NoResultsText shown when search returns no matches (default: "No matches found") |
...
| `Label` | `string` | Label is the display text shown to the user. |
| `Img` | `string` | Img is an optional image URL (e.g. avatar, flag). |
| `Initials` | `string` | Initials are displayed as avatar fallback when Img is empty (e.g. "JD"). |
| `Meta` | `string` | Meta is optional secondary text (e.g. email, description). |
| `Badge` | `string` | Badge is an optional small label rendered next to the option (e.g. "Team", "User"). |
| `BadgeColor` | `string` | BadgeColor determines the badge color variant (info, success, warning, danger, neutral). |

## combobox/v2

```go
import "github.com/araihu/goshtoso/components/combobox/v2"  // package v2
```
