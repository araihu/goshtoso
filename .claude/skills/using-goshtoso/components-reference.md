...
A **Theme** selects the token set for a page or application and is independent of
component identity. A **Primitive** is one meaningful renderable concept. Its
concrete `components.Component` returns its stable **Kind** from `Kind()`. A
**configuration dimension** is one independent choice such as `Tone`,
`Appearance`, `Size`, `Orientation`, `Position`, or `Mode`; it is documentation
vocabulary, not a public `Dimension` type. See
