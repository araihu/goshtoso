...
func TestContentDialogAboveDrawer(t *testing.T) {
	...
	for _, width := range []int{390, 1440} {
		...
		fits, err := dialog.Evaluate(`e=>{const r=e.getBoundingClientRect();return e.matches(':modal') && r.left>=0 && r.right<=innerWidth && r.top>=0 && r.bottom<=innerHeight && (innerWidth>=640 || (r.width===innerWidth && r.height===innerHeight));}`, nil)
		require.NoError(t, err)
