	...
	for _, theme := range []string{"araihu", "minimal"} {
		for _, dark := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s-dark-%t", theme, dark), func(t *testing.T) {
				...
				require.NoError(t, err)
				count, err := page.Locator("#expression-schema").Count()
				require.NoError(t, err)
				require.Zero(t, count)
				require.NoError(t, page.Locator(`a[href="/docs/internationalization/files"]`).First().Click())
				require.NoError(t, page.Locator("#internationalization-files-fragment").WaitFor())
