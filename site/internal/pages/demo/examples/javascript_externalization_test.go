...
import (
	...
	tickerdomain "github.com/araihu/goshtoso/site/internal/examples/ticker"
...
func TestComplexExampleProvidersUseExternalJavaScript(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			view:     ChatApp(chatdomain.Identity{Nick: "Guest", Color: "primary"}),
		},
		{
			name:     "profile",
			provider: `x-data="profileImages"`,
			inlineJS: "Alpine.data('profileImages'",
			view:     ProfileApp(profiledomain.State{}),
		},
		{
			name:     "ticker",
			provider: `x-data="tickerPane()"`,
			inlineJS: "Alpine.data('tickerPane'",
			view:     tickerPane(tickerdomain.InitialSymbols()),
		},
	}
