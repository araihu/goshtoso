...
func TestLandingExtensionsPreloadIsolatedCharts(t *testing.T) {
	...
	require.NoError(t, modulespages.ChartsShowcaseFrame().Render(context.Background(), &frame))
...
func TestChartsShowcasePageUsesAutoRotatingLine3DWithoutOuterShell(t *testing.T) {
	...
	require.NoError(t, modulespages.ChartsShowcasePage().Render(context.Background(), &buffer))
	html := buffer.String()
