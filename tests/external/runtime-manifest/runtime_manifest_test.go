...
func TestExternalConsumerCanBindManifestAndReplacementIdentity(t *testing.T) {
	...
	if len(manifest.Dependencies) != 13 {
		t.Fatalf("dependency count = %d, want 13", len(manifest.Dependencies))
	}
	...
	if manifest.Dependencies[12].Role != assets.RuntimeRoleCodeBlock {
		t.Fatalf("last dependency = %q, want %q", manifest.Dependencies[12].Role, assets.RuntimeRoleCodeBlock)
	}
