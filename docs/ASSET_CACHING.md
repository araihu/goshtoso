...
`assets.Handler()` applies this policy to the library assets it serves and
forces missing or otherwise non-cacheable responses to revalidate. Exporters
using `assets.FS()` or `assets.ReadFile()` own their HTTP route and should apply
the same classifier.
