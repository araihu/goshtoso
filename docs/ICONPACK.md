...
embedded Heroicons package. Consumers can use either the verified Arai Hû
Assets catalog boundary or any icon pack described by a source manifest. Both
paths produce the same generated package and the same `components/icon`
rendering contract.
...
For another icon pack, pass `-source-root` or `-source-archive` together with
`-source-manifest`. The manifest is the pack's small, consumer-owned lock
file: it names the release, source, license, every selected SVG path, exact
sprite symbol, viewBox, and SHA-256. A source manifest without `licenseSha256`
or an icon without `sha256` is rejected. When `spritePath` is absent, each
source SVG is normalized into a safe `<symbol>`; when it is present, the
declared symbols are extracted from that sprite. This supports packs such as
Bootstrap Icons without adding them to Arai Hû Assets or Goshtoso's embedded
catalog.
...
This example uses two Bootstrap Icons SVG files. The source manifest can be
JSON or YAML; JSON is the canonical checked-in form:
