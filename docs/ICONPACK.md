...
Goshtoso release. During local candidate work run `go run ./cmd/iconpack`; once
both public tags exist, consumers should pin the generator to
...
```bash
jq -r '.assets[] | select(.namespace == "brand" or .namespace == "ui") | .canonicalName' \
