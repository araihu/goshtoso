...
For the published Assets v0.2.0 boundary, the command pins the complete
catalog/release/checksums/archive quartet below. An archive with the same inner
files but different outer bytes is rejected. Generated `manifest.json` and
