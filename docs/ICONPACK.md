...
The release workflow uses the immutable Arai Hû Assets `v0.2.0` archive. The
hashes below are the frozen candidate values and must be reverified against the
published Assets release before publication. They identify Assets, not the
Goshtoso release. During local candidate work run `go run ./cmd/iconpack`.
After both public tags exist, consumers should pin the generator to
`github.com/araihu/goshtoso/cmd/iconpack@v0.2.0`.

```bash
# After the Goshtoso v0.2.0 and Assets v0.2.0 tags are public:
